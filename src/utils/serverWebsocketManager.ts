import type Wrapdactyl from "..";
import { EventEmitter } from 'events';
import WebSocket from "ws";
import { serverWebsocketManagerEvents } from "../types";
...
class MyClass extends EventEmitter {
    constructor(readonly Wrapdactyl: Wrapdactyl, id: string) {
        super();

        Object.defineProperty(this, "config", { enumerable: false });
        this.config.origin = Wrapdactyl.config.url;
        this.identifier = id;
        this.fetchWebsocketDetails = Wrapdactyl.client.servers.websocketDetails;
    }

    fetchWebsocketDetails;
    ws?: WebSocket;
    identifier = "";
    config = { 
        socket: "",
    ...
    connect = async () => {
        ...
        const creds = await this.fetchWebsocketDetails(this.identifier);
        ...
        this.ws.on('message', async (data) => {
            ...
            if(message.event === 'auth success') {
                ...
                const creds = await this.fetchWebsocketDetails(this.identifier).catch(() => {});
...
export default (Wrapdactyl: Wrapdactyl) => class example extends EventEmitter implements ServerWebsocketEvents {
